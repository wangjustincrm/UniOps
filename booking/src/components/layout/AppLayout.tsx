...
import { bookingRoutes } from '@/app/routes'

const PORTAL_URL = (import.meta.env.VITE_PORTAL_URL as string | undefined) || 'http://localhost:5174'

// `permission` gates visibility via the EPMS Access Control Matrix.
...
function Sidebar({
  ...
  const brandTagline = branding?.tagline || 'Booking'
  const brandLogo = branding?.logo_data_url || null
  const brandInitials = brandName.split(/\s+/).map((w) => w[0]).join('').slice(0, 2).toUpperCase() || 'BK'

  const { data: matrix } = useRolePermissions()
  const userRole = useBookingAuth((s) => s.user?.role ?? null)
            ...
            <div key={section.title} className="mb-4">
              ...
              {items.map(({ label, href, icon: Icon }) => (
                <NavLink
                  ...
                  className={() => navLinkCls(location.pathname === href || location.pathname.startsWith(href + '/'))}
                >
...
export default function AppLayout() {
  ...
  const isAuthenticated = useBookingAuth((s) => s.isAuthenticated)
  const userId = useBookingAuth((s) => s.user?.id)
  const userRole = useBookingAuth((s) => s.user?.role ?? null)
  ...
  // First nav item the user can access — landing page + pinned home tab.
  ...
  if (!homePath) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-3 text-sm text-neutral-500">
        <p>No access. </p>
        <a href={PORTAL_URL} className="font-medium text-primary-600 hover:underline">Back to Portal.</a>
      </div>
    )
  }

  const homeMeta = deriveTabMeta(bookingRoutes, homePath)
    ...
    <TabStoreProvider options={{ storageKey: 'uniops:booking:tabs:v1', initialTabs, userId }}>
      <div className="relative flex h-screen overflow-hidden bg-[#FAFBFC]">
        ...
        <div className="flex flex-1 flex-col overflow-hidden min-w-0">
          ...
          <TabRouterSync routes={bookingRoutes} />
          <TabBar />
          <main className="relative flex-1 overflow-hidden">
            <TabHost routes={bookingRoutes} pageClassName="h-full" />
          </main>
        </div>
      </div>
    </TabStoreProvider>
