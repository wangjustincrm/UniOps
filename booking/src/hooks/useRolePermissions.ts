...
/**
 * Fetches the Access Control Matrix from epms-api so the Booking sidebar can gate
 * items by the same toggles edited in EPMS → Admin Panel → Access Control Matrix.
 * Falls back to an empty matrix on error (items then hide for non-admins).
 */
