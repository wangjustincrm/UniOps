"""CRUD operations for Department (mdm-api owns writes)."""
import uuid
...
async def get_all(db: AsyncSession, *, active_only: bool = False) -> list[Department]:
    ...
    return list((await db.execute(q.order_by(Department.code))).scalars().all())


async def get_by_id(db: AsyncSession, dept_id: uuid.UUID) -> Department | None:
    return (await db.execute(select(Department).where(Department.id == dept_id))).scalar_one_or_none()


async def get_by_code(db: AsyncSession, code: str) -> Department | None:
    return (await db.execute(select(Department).where(Department.code == code.upper()))).scalar_one_or_none()
