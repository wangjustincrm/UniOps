...
export function initHistory(initial: CellValueMap): History {
  return { stack: [{ cells: initial }], idx: 0 }
}
...
/**
 * Commit a new cell map as one undo step. `updater` receives the current
 * cells and returns the next cells — this lets a single-cell edit AND a
 * whole rectangular paste both land as exactly one snapshot (the paste
 * handler builds the full next map before calling this once).
 *
...
export function commitCells(
  ...
  const next = updater(curr.cells)
  if (sameMap(curr.cells, next)) return history
  const truncated = history.stack.slice(0, history.idx + 1)
  return { stack: [...truncated, { cells: next }], idx: truncated.length }
}
