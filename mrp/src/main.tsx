...
// Portal redirects to `${MRP_URL}/#__session=<base64>` after login. We pluck
...
;(function bootstrapFromPortal() {
  ...
  try {
    ...
    const existing = JSON.parse(localStorage.getItem('mrp-auth') ?? '{}')
    localStorage.setItem('mrp-auth', JSON.stringify({
      ...existing,
      state: {
        ...(existing.state ?? {}),
        token,
