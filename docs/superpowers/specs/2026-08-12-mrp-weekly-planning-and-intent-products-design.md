...
- **保质期硬校验**：**按真实日期差比较，不用"4.33 周/月"近似**——
  ...
  **★修正理由（2026-08-13 实测）**：原文说「近似在 18 个月窗口上累积到整周级偏差」——对 `4.33 周/月` 这个形式**不成立**，实测 2024-2028 各锚点、n=1..18 的最大偏差是 **4.42 天**，不是 7 天。（对「一月=30 天」那个形式才成立，最大 10 天。）**但禁令依然正确**，真正的理由是：在 2025-2027 区间实测有 **20 个真实的 ISO 周一**，近似算法放行而真实日历拒绝——集中在 n=1..3，因为二月短。偏差不需要累积到一整周才害人，它只要跨过某一个具体周的边界就够了。
...
- Excel 导出改为周列 + 月分组表头
- **★周表头右键 = 设检修周**（用户 2026-08-13 要求）：右键周列表头弹上下文菜单（`createPortal` 到 body）
  - `Mark as maintenance week` → 写一条该周 `max_output_qty=0` 的周例外；已是检修周则显示 `Clear maintenance`
  - 门禁 `mrp.param.write`；无权限时菜单项禁用并说明原因
  - 检修周整列灰底 + 扳手图标，一眼可辨
  - **必须另有非右键入口**：表头悬停出一个小箭头按钮。右键菜单在网页里发现性差、键盘用户完全够不到
  - **标记后不自动重排**：弹提示 + 一个 `Recalculate now` 按钮（用户选定）。周例外只影响之后生成/重算的计划，
    已发布 run 是冻结快照不受影响；不自动重算是为了不在计划员没预期时冲掉他的手工调整
  - ★**月内重排已由引擎保证**，无需前端做任何分摊：实测 120 吨/4 周（30/30/30/30）在 W3 设检修后
