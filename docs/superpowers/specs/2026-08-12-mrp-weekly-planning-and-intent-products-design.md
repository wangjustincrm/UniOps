...
④ 每个桶内跑两体制排布（§2.2 / §2.3）。**画布 = 该归属月的全部 4/5 周**，不是「从桶内最早目标周起」
⑤ 桶内装不下的，按周向前溢出（pre-build，可跨月），逐周做保质期硬校验
⑥ 前移触底（当前周）或违反保质期 → 标 capacity_gap，不静默丢弃
```

**★保质期闸门作用于每一个落placed周，不只作用于第⑤步的溢出**（2026-08-13 修正）。原文只在⑤提闸门，于是「画布 = 整月」时，落在目标周之前的那些周（`weeks_early > 0`）会**完全不过闸门**——这是个潜在的报废写单 bug。两条必须同时成立：画布是整月（否则 P2/P3 失效，见下），且每一个早于目标周的落点都要过闸门。

**★为什么画布必须是整月**：`lead_weeks=4` 的默认配置下，每个桶通常只收到**一个**需求月（10 月需求的目标周落在 9 月末 → 归 9 月桶；11 月需求 → 10 月桶）。若画布只从「桶内最早目标周」起算，实测 8 个活跃桶里有 4 个画布只剩**一周**，P2 摊平与 P3 单品周在那里结构性失效，且一个零缺口的计划里 39% 的行会被标成 `is_prebuild`（lead=0 时 62%）——正是 §2.6 要避免的「每行都在报提前生产，告警失去意义」。

**★`lead_weeks=0` 的含义**：复现月引擎的「不提前」行为 = 产量落在**需求月之内**，不是「只能落在需求月最后一周」。月引擎的放置单位就是月。
...
- **保质期硬校验**：**按真实日期差比较，不用"4.33 周/月"近似**——
  ...
  **★修正理由（2026-08-13 实测）**：原文说「近似在 18 个月窗口上累积到整周级偏差」——对 `4.33 周/月` 这个形式**不成立**，实测 2024-2028 各锚点、n=1..18 的最大偏差是 **4.42 天**，不是 7 天。（对「一月=30 天」那个形式才成立，最大 10 天。）**但禁令依然正确**，真正的理由是：在 2025-2027 区间存在**真实的 ISO 周一被近似算法放行、被真实日历拒绝**，集中在 n=1..3（二月短）。偏差不需要累积到一整周才害人，它只要跨过某一个具体周的边界就够了。★**别在文档里引用一个具体条数**：实测该计数强烈依赖枚举口径（安全余量作用在**未取整**的近似期限上，还是先取整再乘），两次独立测量分别得到 20 与 8。要写数字就把枚举方式一并写死，否则下一个人复现不出来会以为结论是错的。
...
- **★点周表头 → 右侧抽屉设检修周**（用户 2026-08-13 要求；**不用右键**，与现有「点计划格 → AdjustDrawer」同一套交互）
  ...
  - 抽屉内容：周标签与日期区间、`Maintenance week (no production)` 勾选框（写/清该周 `max_output_qty=0` 的周例外）、原因文本框
  - 门禁 `mrp.param.write`；无权限时勾选框禁用并写明原因（读权限仍可打开抽屉看状态）
  - 检修周整列灰底 + 扳手图标，一眼可辨
  - **标记后不自动重排**：保存后 toast 里带一个 `Recalculate now` 按钮（用户选定方案 1）。周例外只影响之后生成/重算的
    计划，已发布 run 是冻结快照不受影响；不自动重算是为了不在计划员没预期时冲掉他的手工调整
  - ★**月内重排已由引擎保证**，前端不做任何产量分摊：实测 120 吨/4 周（30/30/30/30）在 W3 设检修后
    自动变成 W1/W2/W4 各 40，零缺口。「向前溢出」只用于整桶装不下的部分
