...
| **损耗率（用户决策 2026-08-04）** | **BOM 视为精准数据（无损耗），损耗在 MRP 运算时按系统配置的损耗率统一放大需求量。只设两种：`raw_material_loss_rate`（原料损耗率）与 `packaging_loss_rate`（包材损耗率）。** 适用判定按物料类别：包材（CP*/item_type=packaging）用包材损耗率，其余一切被消耗物料（CR 原辅料、CS/CW 半成品粉）用原料损耗率。展开时 `需求量 = 基础用量 × (1 + 适用损耗率)`，只施加一次（逐层展开时在各层组件上按其类别取率，不累乘祖先层的率）。<br>⚠️**上线前必须核对包材损耗率的初值**：实测 NC 的包装 BOM **已内含损耗**（S0093 规格 700g、批产出 420kg → 理论 600 只罐而 BOM 写 610 只 = +1.67%；纸箱理论 100 个而 BOM 写 105 个 = +5%），制粉/干混 BOM 则无损耗（用户确认）。故包材损耗率初值应设 **0**，否则重复放大；待 NC 把包装 BOM 改为精准用量后，再改为真实值——**代码无需改动**。此说明须显示在参数维护界面上，防止有人看到 0 就顺手补值。<br>**禁止**把损耗写回 `bom_lines.scrap_rate`（那是 NC 的字段、本实例恒为 0，不是我们的参数） |
