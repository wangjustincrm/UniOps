...
### 14.7 红字发生额虚增 —— `_net_side` 逐行翻转(2026-07-17 新增,归本分支一起发)

**症状(用户实测)**:NC 科目余额表 2026-06,660101「Selling expenses(fix)」本期借方发生额 = **579,025.62**,我们的科目余额表 = **971,338.12**(差 392,312.50);660102(无红字)完全一致;非末级 6601 汇总后同样偏 392,312.50。**期末余额两边都对**(都显示 Balanced)。

**根因**:NC 的红字(冲销)是**在原栏记负数**——借方红字 = `debitamount` 为负,贷方红字 = `creditamount` 为负;NC 科目余额表的「本期借方/贷方发生额」= 各栏 `localdebitamount`/`localcreditamount` 的**带符号求和**(负数在本栏内相抵)。而 [`nc_sync.py`](../../../finance-api/app/services/nc_sync.py) 的 `_net_side(dr,cr)=n=dr-cr; (n,0) if n>=0 else (0,-n)` 把每行按净额归到**单边**,于是一笔红字贷方(`localcredit<0`)被算成 `n=0-(负)=正`→**翻到借方**,虚增借方发生额;红字借方翻到贷方。

**直查 NC 坐实(660101,2026-06,393 行)**:24 笔红字借方(合计 −69,898.74)+ 66 笔红字贷方(合计 −322,413.76)。算术精确:`Σ正常借(648,924.36)+|红字贷|(322,413.76)=971,338.12` = 我们的镜像;`Σ带符号 localdebit = 579,025.62` = NC。

**修复**(`transform()` 用新 `_orig_side` 取代 `_net_side`):**每行保留在原栏、符号原样**——`orig_debit=debitamount`、`orig_credit=creditamount`、local 同理(红字即负数)。因 `Σ(ldr-lcr)` 不变,**期末余额/balanced 完全不变**;仅发生额 gross 与凭证头合计改为与 NC 一致。

**关键约束(排查中发现,正是 `_net_side` 当初存在的真实原因)**:表上有 CHECK `ck_jv_lines_one_side = NOT(orig_debit>0 AND orig_credit>0)`——**只禁两边同为正**,负数(红字)不受限。直查 CRM 账套 315,610 行:**两边同为正仅 1 行**(任一符号两边非零共 2 行),红字单边负数 **71,262 行**。故 `_orig_side` **只对「两边同为正」这一种情形按净额归一**(满足约束、保住 go-forward 单边不变式),其余(含全部红字)一律原栏原值。这 1 行的净额化影响可忽略。

**blast radius**:jv_lines 会出现负数金额(即红字,NC 原样)。全仓 grep 无任何下游对金额非负的假设;`account_balance`/`expand`/`vouchers`/`budget_actual` 均为 `Σ` 聚合,带符号即得 NC 口径;JV 明细/列表按字符串显示,负数即红字,正确。

**生效前提**:属存量数据纠正,**须随 §14.5 的全量重灌一起生效**(不重灌则历史行仍是翻转值)。

### 14.8 期初/期末口径:缺损益类年结(定性完成,本分支不修,单列)

**症状**:6601/660101 的期末对不平 NC 直觉预期(660101 closing 我们显 −349,824.95,NC 科目余额表显 Balanced/空)。

**定性(直查 NC GL_DETAIL,CRM 账套,2026-07-17)**:
- `accountcode='6601'` 的 2454 行直接分录**是 NC 真实底账**,期间 **2020-10..2024-07**——6601 早年是记账科目,约 2024/2025 才重构为末级 660101/660102。
- 逐年净额:6601 在 **2024 年 net=+349,824.95**、660101 **2024 年 net=−349,824.95**(一笔跨科目重分类,成对,账套内净额 0);其余完整年度两者各自净额=0(损益 PLCF 期间结转在起作用,我们也导入了)。
- **关键:我们镜像的累计期初与 NC GL_DETAIL 同口径完全一致**(6601 +349,824.95 / 660101 −349,824.95 / 660102 0)。**数据无误,导入忠实。**

**根因(报表口径,非数据)**:我们的 `account_balance` 期初 = `fiscal_period < period` 的**自建账以来累计**;而 NC 科目余额表对**损益类(收入/费用)科目按会计年度重置**(年末结转留存收益,次年期初归零)。那笔 2024 的 ±349,824.95 被 NC 结转结平、次年不再背,我们无年结逻辑故一直累计到 2026。

**范围**:影响**所有有跨年余额的损益类科目**的期初/期末;**发生额不受影响**(已随 §14.7 对齐)。

**修复方向(另开任务,需 brainstorm)**:给 `account_balance` 加**会计年度边界的期初**——损益类科目期初 = 从本会计年度起(而非建账起)累计,或显式建模期间损益结转。属独立特性,**不阻断本次 rollup + §14.7 发布**。
