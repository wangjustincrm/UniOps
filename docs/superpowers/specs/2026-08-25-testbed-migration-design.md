...
**★ 2026-08-26 实证**: 恢复后该表里躺着 `mail.canadaroyalmilk.com` / 587 /
`notice@canadaroyalmilk.com` —— **公司真实邮件服务器的完整可用凭据**。
这不是理论推演。中和后正面确认 `WHERE smtp_host IS DISTINCT FROM 'mailhog'` 为 0。
...
初版设计据 2026-08-07 的记录写道: 本地 164 表 vs 生产 141 表, 多出的 23 张全是 MRP 的。

**★ 2026-08-26 实测: 该差异已消失。** 生产快照含 **24 张 mrp 相关表**, 总表数 179,
与本地一致 —— MRP 于 2026-08-08 `fe636df` 上生产之后, 两边就对齐了。
因此"单独导出 MRP 表再灌回"那一步**不再需要**。

用 `pg_restore -l` 直接查 dump 目录即可确认, 不必先恢复。
**教训: 记忆里的差异描述会随发布而过期, 落地前实测一次。**
...
3. **§2.10 那个 `vendor_credits` 陷阱自动消失。**

**★ 2026-08-26 实证 D5 的收益**: 恢复生产快照后, **7 个服务的 `alembic current` 全部等于 `heads`**,
一次 `upgrade` 都不需要。若当初钉了 main, 就会落入"代码要表而快照没表"的错配。 该陷阱的成因是"代码钉 main(含未发布迁移)
...
├── 栈 uniops-dev1    5273-5279 / 8100-8111   ← crmadmin, 自己的 worktree
├── 栈 uniops-dev2    5373-5379 / 8200-8211   ← amir
...
2. **F12 Network 确认请求打的是 `<IP>:8000` 而不是 `localhost:8000`** —— LAN 化唯一真正会翻车的地方
   ...
   **★ 2026-08-26 用程序化方式取得了更强的证据**: 从另一台机器抓 Vite 转换后的模块
   (`curl http://<IP>:5174/src/pages/LoginPage.tsx`), 可以直接看到 Vite 注入在首行的
   `import.meta.env` 对象里写着 `"VITE_EPMS_API_URL": "http://10.10.50.64:8000"`。
   源码里的 `|| "http://localhost:8000"` 回落因此**永不触发**。
   比肉眼看 Network 面板更确定, 且可复现。
