...
4. **双重付款防护**（NC 切换后仍自动生成 `ap_paybill`）：设计前提"NC 自动付款单 ≠ 真实打款、真实打款只在 UniOps"。**流程约定，非代码**，落地前须财务确认 NC 不真出款。
...
- 不导入 NC 发票/结算单/付款单。
- 不建物料操作级主数据。
- 不同步银行信息（付款沿用现有 vendor/QBO 机制）。
- 不碰现有 EPMS 采购（其他类别）与发票/匹配/PA/付款代码——只做加法。

## 14. 待验证/开放项（落地前确认）

1. Firmus `suppliercode` == NC `bd_supplier.code`？（供应商解析成立的前提）
2. NC `forderstatus` 的 0/2、`fbillstatus` 的 0 精确含义（自由态/关闭/变更中）；本设计只取 3。
3. 付款所需银行信息落在哪（vendor 无银行列；现有 PA 付款如何取银行信息，NC PA 沿用同机制）。
4. 双重付款流程约定（财务确认 NC 自动付款单不真实出款）。
5. 前端 type-1 开关：仅对"NC 镜像来源"放开展示，还是整体启用该类别（需与现有禁用意图对齐）。
6. epms-api 引入 `oracledb` 依赖 + 生产网络连通性（50.65→95.67:1521）。
