...
def _meta(att: PrAttachment) -> AttachmentMeta:
...
async def list_attachments(pr_id: uuid.UUID, db: SessionDep, _: CurrentUserPayload):
    result = await db.execute(
        select(PrAttachment).where(PrAttachment.pr_id == pr_id).order_by(PrAttachment.created_at)
    )
...
async def upload_attachment(
    pr_id: uuid.UUID, file: UploadFile,
...
async def download_attachment(
    pr_id: uuid.UUID, att_id: uuid.UUID,
    db: SessionDep, _: CurrentUserPayload, token: BearerToken,
):
    result = await db.execute(
        select(PrAttachment).where(PrAttachment.id == att_id, PrAttachment.pr_id == pr_id)
    )
