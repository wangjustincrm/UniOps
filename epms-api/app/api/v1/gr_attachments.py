...
def _meta(att: GrAttachment) -> AttachmentMeta:
...
async def list_attachments(gr_id: uuid.UUID, db: SessionDep, _: CurrentUserPayload):
    result = await db.execute(
        select(GrAttachment).where(GrAttachment.gr_id == gr_id).order_by(GrAttachment.created_at)
    )
...
async def upload_attachment(
    gr_id: uuid.UUID, file: UploadFile,
...
async def download_attachment(
    gr_id: uuid.UUID, att_id: uuid.UUID,
    db: SessionDep, _: CurrentUserPayload, token: BearerToken,
):
    result = await db.execute(
        select(GrAttachment).where(GrAttachment.id == att_id, GrAttachment.gr_id == gr_id)
    )
