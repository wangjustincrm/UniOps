"""PA attachment endpoints."""
import asyncio
import uuid

from fastapi import APIRouter, HTTPException, UploadFile, status
from fastapi.responses import Response
from pydantic import BaseModel
from sqlalchemy import select

from app.core.config import settings
from app.core.deps import BearerToken, CurrentUserPayload, SessionDep
from app.crud.pa import get_by_id as get_pa
from app.crud.signatories import approval_signatories
from app.models.config import CompanyConfig
from app.models.pa_attachment import PaAttachment
from app.services.attachment_helper import delete_from_file_server, proxy_download, upload_to_file_server
from app.services.pdf_pa import generate_pa_pdf

router = APIRouter(prefix="/pa/{pa_id}/attachments", tags=["pa-attachments"])

MAX_FILE_SIZE = 25 * 1024 * 1024  # 25 MB

# Statuses at/after which an approved-document PDF is meaningful (regenerate-able).
_PDF_STATUSES = {"approved", "processed"}


class AttachmentMeta(BaseModel):
    id: uuid.UUID
    filename: str
    content_type: str
    file_size: int
    created_at: str
    download_url: str | None = None

    model_config = {"from_attributes": True}


def _meta(att: PaAttachment) -> AttachmentMeta:
    dl_url = f"{settings.FILE_SERVER_URL}/files/{att.storage_key}" if att.storage_key else None
    return AttachmentMeta(
        id=att.id, filename=att.filename,
        content_type=att.content_type, file_size=att.file_size,
        created_at=att.created_at.isoformat(), download_url=dl_url,
    )


@router.get("", response_model=list[AttachmentMeta])
async def list_attachments(pa_id: uuid.UUID, db: SessionDep, _: CurrentUserPayload):
    result = await db.execute(
        select(PaAttachment).where(PaAttachment.pa_id == pa_id).order_by(PaAttachment.created_at)
    )
    return [_meta(r) for r in result.scalars().all()]


@router.post("", response_model=AttachmentMeta, status_code=status.HTTP_201_CREATED)
async def upload_attachment(
    pa_id: uuid.UUID, file: UploadFile,
    db: SessionDep, user: CurrentUserPayload, token: BearerToken,
):
    pa = await get_pa(db, pa_id)
    if pa is None:
        raise HTTPException(status_code=404, detail="PA not found")
    data = await file.read()
    if len(data) > MAX_FILE_SIZE:
        raise HTTPException(status_code=413, detail="File exceeds 25 MB limit")

    storage_key = await upload_to_file_server(
        data, file.filename or "attachment",
        file.content_type or "application/octet-stream",
        "pa", pa_id, token,
    )
    att = PaAttachment(
        pa_id=pa_id,
        filename=file.filename or "attachment",
        content_type=file.content_type or "application/octet-stream",
        file_size=len(data),
        storage_key=storage_key,
    )
    db.add(att)
    await db.flush()
    await db.refresh(att)
    return _meta(att)


@router.post("/regenerate-pdf", response_model=AttachmentMeta)
async def regenerate_pdf(
    pa_id: uuid.UUID,
    db: SessionDep, user: CurrentUserPayload, token: BearerToken,
):
    """(Re)generate the approved-PA PDF and (re)attach it.

    Backfills the PDF on PAs that reached ``approved`` without passing through
    the live approval action (e.g. PMS-imported PAs), or refreshes it after a
    template/logo change. Replaces any existing ``<pa_number>.pdf`` attachment.
    """
    pa = await get_pa(db, pa_id)
    if pa is None:
        raise HTTPException(status_code=404, detail="PA not found")
    if pa.status not in _PDF_STATUSES:
        raise HTTPException(status_code=409, detail="PDF is only available once the PA is approved")

    cfg = (await db.execute(select(CompanyConfig).limit(1))).scalar_one_or_none()
    company_name = cfg.name if cfg else "EPMS"
    filename = f"{pa.pa_number}.pdf"
    requester_name, approvals = await approval_signatories(db, "pa", pa_id, pa.created_by)
    loop = asyncio.get_event_loop()
    pdf_bytes = await loop.run_in_executor(
        None, generate_pa_pdf, pa, company_name,
        cfg.pdf_templates if cfg else None,
        cfg.logo_data_url if cfg else None,
        requester_name, approvals,
    )

    existing = (await db.execute(
        select(PaAttachment).where(
            PaAttachment.pa_id == pa_id,
            PaAttachment.filename == filename,
        )
    )).scalars().all()
    for att in existing:
        if att.storage_key:
            await delete_from_file_server(att.storage_key, token)
        await db.delete(att)
    await db.flush()

    storage_key = await upload_to_file_server(
        pdf_bytes, filename, "application/pdf", "pa", pa_id, token,
    )
    att = PaAttachment(
        pa_id=pa_id, filename=filename,
        content_type="application/pdf", file_size=len(pdf_bytes),
        storage_key=storage_key,
    )
    db.add(att)
    await db.flush()
    await db.refresh(att)
    return _meta(att)


@router.get("/{att_id}/download")
async def download_attachment(
    pa_id: uuid.UUID, att_id: uuid.UUID,
    db: SessionDep, _: CurrentUserPayload, token: BearerToken,
):
    result = await db.execute(
        select(PaAttachment).where(PaAttachment.id == att_id, PaAttachment.pa_id == pa_id)
    )
    att = result.scalar_one_or_none()
    if att is None:
        raise HTTPException(status_code=404, detail="Attachment not found")
    if att.storage_key:
        return await proxy_download(att.storage_key, token)
    if att.file_data:
        return Response(
            content=att.file_data, media_type=att.content_type,
            headers={"Content-Disposition": f'attachment; filename="{att.filename}"'},
        )
    raise HTTPException(status_code=410, detail="File data unavailable")


@router.delete("/{att_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_attachment(
    pa_id: uuid.UUID, att_id: uuid.UUID,
    db: SessionDep, _: CurrentUserPayload, token: BearerToken,
):
    result = await db.execute(
        select(PaAttachment).where(PaAttachment.id == att_id, PaAttachment.pa_id == pa_id)
    )
    att = result.scalar_one_or_none()
    if att is None:
        raise HTTPException(status_code=404, detail="Attachment not found")
    if att.storage_key:
        await delete_from_file_server(att.storage_key, token)
    await db.delete(att)
    await db.flush()
