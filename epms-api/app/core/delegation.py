...
# Mirrors _PERSONAL_APPROVAL_ROLES in app/crud/task.py. Defined literally rather
# than imported to avoid a cycle: app.crud.task imports from this module.
...
async def delegated_broadcast_roles(
    ...
    # is_active is filtered on BOTH halves of the union (mirrors
    # access_scope.role_holder_ids): a deactivated delegator is not a role
    # holder anywhere else in the system, so neither their PRIMARY role
    # (users.role) nor their ADDITIONAL roles (user_roles) should leak into
    # a delegate's broadcast-role set — surfacing role-pool tasks to the
    # delegate on behalf of someone no longer treated as holding the role.
