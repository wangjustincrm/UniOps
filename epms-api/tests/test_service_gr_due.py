...
# Relative, not a fixed date. It was date(2026, 8, 21), which made
# test_escalation_only_fires_past_the_manager_threshold pass for three days and
# fail forever after: that test goes through run_service_gr_due(), which takes
# no `today` and so uses the real one, while the fixtures were dated against
# 2026-08-21. A PO seeded "one day overdue" was 26 days overdue by the time the
# sweep looked at it, and escalated exactly as it should have.
#
# The other twelve uses pass today=TODAY into scan_due_service_pos explicitly,
# so they were never affected and still are not.
TODAY = date.today()
