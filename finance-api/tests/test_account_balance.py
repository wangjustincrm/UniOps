...
async def _posted_partner_credit_event(db, account, amount, *, cc_id, ba_id, partner_id, partner_name, period):
    """A pure-credit (carry-forward) line on `account` — local_debit = 0."""
    occurred = datetime(int(period[:4]), int(period[5:7]), 15, tzinfo=timezone.utc)
    line = {"line_role": "purchase_expense", "account_code": account,
            "credit": Decimal(amount), "currency": "CAD", "cost_center_id": cc_id,
            "partner_id": partner_id, "partner_name": partner_name,
            "aux": {"income_expense_item": {"value_id": ba_id, "value_text": "X"}}}
    await emit_event(
        db, source_service="finance", source_doc_type="ap_invoice",
        source_doc_id=uuid.uuid4(), source_doc_number="CF-1", event_type="accrual",
        occurred_at=occurred, prepared_by=uuid.uuid4(),
        lines=[line, {"line_role": "purchase_expense", "account_code": "2000",
                      "debit": Decimal(amount), "currency": "CAD"}])
...
async def test_nc_partner_monthly_and_vouchers(db_session):
    ...
    # a pure-credit carry-forward for a 3rd partner -> must NOT appear (debit-only)
    await _posted_partner_credit_event(db_session, "5101", "500.00", cc_id=cc, ba_id=it, partner_id=pC, partner_name="CarryFwd", period="2026-03")
    await jv_crud.backfill_posted_jvs(db_session)
    ...
    assert "CarryFwd" not in by                            # credit-only partner dropped
    ...
    # credit carry-forward line excluded from the drill too
    vc = await ab.nc_partner_vouchers(db_session, it, 2026, 3, cc, pC)
    assert vc["rows"] == []
