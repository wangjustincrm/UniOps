"""finance-api data-maintenance admin endpoints (mounted at /finance/v1/admin)."""
import uuid
...
def require_system_admin(payload: Annotated[dict, Depends(get_token_payload)]) -> dict:
    if payload.get("role") != "system_admin":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="system_admin required")
    return payload


AdminUser = Annotated[dict, Depends(require_system_admin)]
