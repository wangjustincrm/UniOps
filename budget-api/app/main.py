"""budget-api FastAPI application factory."""
import logging
import time
...
async def lifespan(app: FastAPI):  # noqa: ARG001
    ...
    # Ensure single-row settings exists
    async with AsyncSessionLocal() as session:
        existing = await session.execute(select(BudgetSettings).limit(1))
        if existing.scalar_one_or_none() is None:
            session.add(BudgetSettings())
            await session.commit()
            logger.info("Seeded default BudgetSettings row")
    yield
...
def create_app() -> FastAPI:
    ...
    # Also expose health checks at root so docker-compose healthchecks
    # and check-health.sh (`/health`) work without the /api/v1 prefix.
